...
// 问最少还需要建设多少条道路？ 
//    输入格式：测试输入包含若干测试用例。每个测试用例的第1行给出两个正整数，分别是城镇数目N（N<1000）和道路数目M，
// 随后的M行对应M条道路，每行给出一对正整数，分别是该条道路直接连通的两个城镇的编号。为简单起见，城镇从1到N编号。
// 注意两个城市之间可以有多条道路相通，也就是说：
//    3 3
//    1 2
//    1 2
//    2 1
...
{  if (x!=parent[x])        //不等于自身说明自身在某个树上
      parent[x]=Find(parent[x]);	//路径压缩，并未改变树的结构而是继续往上找根节点
   return parent[x];
}
...
int main()
{  while(~scanf("%d",&n))  //输入N,%d代表十进制有符号整数
   {  if(n==0) break;  //N为0则停止输出
      scanf("%d",&m); //输入M的值m
      Init();				//执行初始化
      ...
      int ans=0;
      for (int i=1;i<=n;i++)   	//根据n个元素，求子树个数ans
          if (parent[i]==i)     //parent[i]==i代表此元素为根，则多一个子树
             ans++;
      printf("%d\n",ans-1);       	//结果为ans-1，2个子树需要多修一条路
   }
}
